...
def main():
    ...
    try:
        ...
        spg_common.log(
            f"Something went wrong while preparing the upload: {
                str(exception)}")
        sys.exit(1)
    ...
    if not uploader.check_location(location):
        spg_common.log(
            f"The specified location if not valid for this hosting type.")
        sys.exit(1)
    ...
    try:
        ...
        spg_common.log(
            f"Something went wrong while uploading the gallery: {
                str(exception)}")
        sys.exit(1)
